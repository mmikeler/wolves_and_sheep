...
EasyC.prototype.drawLines = function (obj) {
	var ctx = this.canvas.getContext("2d");
	...
	for (var i = 0; i < obj.corners.length; i++) {
		var corner = [
			width * obj.corners[i][0],
			height * obj.corners[i][1],
			width * obj.corners[i][2],
			height * obj.corners[i][3],
		];

		ctx.moveTo(corner[0], corner[1]);
		ctx.lineTo(corner[2], corner[3]);
	}
	ctx.strokeStyle = obj.stroke.stroke
	ctx.stroke()
	ctx.closePath();
}
